...
const readFeedsByUserId = `-- name: ReadFeedsByUserId :many
SELECT id, created_at, updated_at, name, url, user_id, last_fetched_at FROM feeds WHERE user_id = $1
`

func (q *Queries) ReadFeedsByUserId(ctx context.Context, userID string) ([]Feed, error) {
	rows, err := q.db.QueryContext(ctx, readFeedsByUserId, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
